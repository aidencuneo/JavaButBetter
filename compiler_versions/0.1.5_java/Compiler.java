...
public class Compiler {
    ...
    public static String compileExpr(ArrayList<Token> tok, boolean nested) {
        ...
        // Logical or
        else if ((f = findAnyToken(tok, List.of("||", "or"))) != -1) {
            String lhs = compileExpr(Util.select(tok, 0, f));
            String rhs = compileExpr(Util.select(tok, f + 1));
        ...
        // Logical and
        else if ((f = findAnyToken(tok, List.of("&&", "and"))) != -1) {
            String lhs = compileExpr(Util.select(tok, 0, f));
            String rhs = compileExpr(Util.select(tok, f + 1));
            System.out.println(lhs + " && " + rhs + " ... " + Util.d(tok));
        ...
        // Equality operators
            ...
            String lhs = compileExpr(Util.select(tok, 0, f));
            String rhs = compileExpr(Util.select(tok, f + 1));
            // System.out.println(lhs + " " + oper + " " + rhs + " ... " + Util.d(tok));
            out += lhs + " " + oper + " " + rhs;
        }

        // Relational and type testing operators
        else if ((f = findAnyToken(tok, List.of("<", ">", "<=", ">=", "is"))) != -1) {
            String oper = tok.get(f).value;
