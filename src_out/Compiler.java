...
public class Compiler {
    ...
    public static CompResult compileFile(String className, String code) {
        ...
        for (var i : LangUtil.asIterable(Extensions.len(lines))) {
            ...
            var compRes = new CompResult(classes, startTemplate, endTemplate, lineMap).getCompiledCode();
    ...
    public static String compileExpr(ArrayList<Token> tok, boolean nested) {
        ...
        if (LangUtil.isTruthy(Extensions.operEq(Extensions.operGetIndex(tok, 0).type, Token.Type.INLINE))) {
        ...
        else if (LangUtil.isTruthy((LangUtil.isTruthy(Extensions.operGe(((f = findTokenType(tok, Token.Type.ASSIGN))), 1))) ? (Extensions.operEq(Extensions.operGetIndex(tok, Extensions.operSub(f, 1)).type, Token.Type.SQUARE_EXPR)) : (Extensions.operGe(((f = findTokenType(tok, Token.Type.ASSIGN))), 1)))) {
            var indexExpr = LangUtil.slice(Extensions.operGetIndex(tok, Extensions.operSub(f, 1)).value, 1, Extensions.operUnarySub(1), 1);
            var index = compileExpr(Tokeniser.tokLine(indexExpr));
            var lhs = compileExpr(LangUtil.slice(tok, null, Extensions.operSub(f, 1), 1));
        ...
        else if (LangUtil.isTruthy(!((boolean) Extensions.operEq(((f = findAnyToken(tok, LangUtil.listOf("<", ">", "<=", ">=", "as", "is", "isnt", "in", "instof", "instanceof")))), Extensions.operUnarySub(1))))) {
            var oper = Extensions.operGetIndex(tok, f).value;
            ...
            if (LangUtil.isTruthy(Extensions.operIn(oper, LangUtil.listOf(">", ">=", "<", "<=")))) {
                ...
                while (LangUtil.isTruthy(Extensions.operGt(Extensions.len(tok), 1))) {
                    ...
                    var operName = (LangUtil.isTruthy(Extensions.operGetIndex((new HashMap<>(Map.ofEntries(Map.entry(">", "Gt"), Map.entry(">=", "Ge"), Map.entry("<", "Lt"), Map.entry("<=", "Le")))), oper))) ? (Extensions.operGetIndex((new HashMap<>(Map.ofEntries(Map.entry(">", "Gt"), Map.entry(">=", "Ge"), Map.entry("<", "Lt"), Map.entry("<=", "Le")))), oper)) : ("");
                    Extensions.operShl(stack, Extensions.operAdd(Extensions.operAdd(Extensions.operAdd(Extensions.operAdd(Extensions.operAdd(Extensions.operAdd("Extensions.oper", operName), "("), lhs), ", "), rhs), ")"));
                }
            ...
            else if (LangUtil.isTruthy(Extensions.operIn(oper, LangUtil.listOf("instof", "instanceof")))) {
                out = Extensions.operAdd(out, (Extensions.operAdd(Extensions.operAdd(Extensions.operAdd(Extensions.operAdd("", (lhs)), " instanceof "), (rhs)), "")));
            }
            else {
            ...
            if (LangUtil.isTruthy(Extensions.operEq(Extensions.operGetIndex(tok, f).type, Token.Type.EXPR))) {
                ...
                else if (LangUtil.isTruthy(Extensions.operIn(name, LangUtil.listOf("round", "roundstr", "sum", "min", "max", "none", "any", "all", "exit")))) {
        ...
        else if (LangUtil.isTruthy((LangUtil.isTruthy(Extensions.operEq(Extensions.len(tok), 2))) ? ((LangUtil.isTruthy(Extensions.operIn(Extensions.operGetIndex(tok, 0).value, LangUtil.listOf("f", "F")))) ? (Extensions.operEq(Extensions.operGetIndex(tok, 1).type, Token.Type.STRING)) : (Extensions.operIn(Extensions.operGetIndex(tok, 0).value, LangUtil.listOf("f", "F")))) : (Extensions.operEq(Extensions.len(tok), 2)))) {
            var quote = Extensions.operGetIndex(Extensions.operGetIndex(tok, 1).value, 0);
            var s = LangUtil.slice(Extensions.operGetIndex(tok, 1).value, 1, Extensions.operUnarySub(1), 1);
            var buffer = Extensions.operAdd("", quote);
            var insertBuffer = "";
            var cb = 0;
            for (var c : LangUtil.asIterable(s)) {
                if (LangUtil.isTruthy(Extensions.operEq(c, '{'))) { ++ cb; }
                if (LangUtil.isTruthy(Extensions.operGt(cb, 0))) {
                    insertBuffer = Extensions.operAdd(insertBuffer, (c));
                }
                else {
                    buffer = Extensions.operAdd(buffer, (c));
                }
                if (LangUtil.isTruthy(Extensions.operEq(c, '}'))) {
                    -- cb;
                    if (LangUtil.isTruthy((LangUtil.isTruthy(Extensions.operEq(cb, 0))) ? (Extensions.operGe(Extensions.len(insertBuffer), 2)) : (Extensions.operEq(cb, 0)))) {
                        insertBuffer = LangUtil.slice(insertBuffer, 1, Extensions.operUnarySub(1), 1);
                        if (LangUtil.isTruthy(Extensions.operEq(insertBuffer, "{}"))) {
            ...
            buffer = Extensions.operAdd(buffer, (quote));
            out = Extensions.operAdd(out, (compileExpr(Tokeniser.tokLine(buffer))));
        }
