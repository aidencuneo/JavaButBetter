...
public class Compiler {
    ...
    public static String compileExpr(ArrayList < Token > tok , boolean nested) {
        ...
        if (LangUtil.isTruthy(!Extensions.operEq(((f = findTokenType(tok, Token.Type.COMMA))), - 1))) {
        ...
        else if (LangUtil.isTruthy(!Extensions.operEq(((f = findTokenRev(tok, "??"))), - 1))) {
            String lhs = compileExpr(LangUtil.slice(tok, null, f, 1));
            String rhs = compileExpr(LangUtil.slice(tok, f + 1, null, 1));
            out += "((" + lhs + ") != null) ? (" + lhs + ") : (" + rhs + ")";
        }
        else if (LangUtil.isTruthy(!Extensions.operEq(((f = findAnyTokenRev(tok, List.of("||", "or")))), - 1))) {
            String lhs = compileExpr(LangUtil.slice(tok, null, f, 1));
            String rhs = compileExpr(LangUtil.slice(tok, f + 1, null, 1));
            out += "(LangUtil.isTruthy(" + lhs + ")) ? (" + lhs + ") : (" + rhs + ")";
        }
        else if (LangUtil.isTruthy(!Extensions.operEq(((f = findAnyTokenRev(tok, List.of("&&", "and")))), - 1))) {
            String lhs = compileExpr(LangUtil.slice(tok, null, f, 1));
            String rhs = compileExpr(LangUtil.slice(tok, f + 1, null, 1));
        ...
        else if (LangUtil.isTruthy(!Extensions.operEq(((f = findAnyTokenRev(tok, List.of("==", "!=", "===", "!==")))), - 1))) {
        ...
        else if (LangUtil.isTruthy(!Extensions.operEq(((f = findAnyTokenRev(tok, List.of("<", ">", "<=", ">=", "is")))), - 1))) {
            String oper = Extensions.operGetIndex(tok, f).value;
            ...
            if (LangUtil.isTruthy((LangUtil.isTruthy(Extensions.operEq(oper, "+"))) ? (lhs) : (Extensions.operEq(oper, "+")))) {
                ...
                out += "Extensions.operUnaryAdd(" + rhs + ")";
            }
            else if (LangUtil.isTruthy(Extensions.operEq(oper, "-"))) {
                out += "Extensions.operUnarySub(" + rhs + ")";
            }
        }
        else if (LangUtil.isTruthy(!Extensions.operEq(((f = findAnyTokenRev(tok, List.of("*", "/", "%")))), - 1))) {
            String oper = Extensions.operGetIndex(tok, f).value;
        ...
        LangUtil.println(mainClassName + " --> " + Extensions.len(out));
        if (LangUtil.isTruthy(Extensions.len(out) > 1000)) {
            LangUtil.println((tok + ", " + out));
        }
        return out.trim();
    }
